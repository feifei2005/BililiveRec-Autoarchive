...
启用 `transcode.limit_resolution` 或管理页的“超高分辨率自动降采样”后，只有宽度严格大于 2560 或高度严格大于 1440 的视频才会缩放。程序在 `0.75x`（2K 类）和 `0.5x`（4K 类）中选择短边最接近 1080 的档位，并保持原宽高比和偶数尺寸。例如 `2560x1600 → 1920x1200`、`3840x2160 → 1920x1080`；`1600x1280`、`2560x1080` 和 `2560x1440` 保持不变。主池使用 `scale_qsv mode=hq`，NV12 深度处理池使用 Lanczos。
